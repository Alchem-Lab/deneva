...
class MemstoreBPlusTree  : public Memstore {
 ...
 public:
  ...
  inline uint slotAtLeaf(uint64_t key, LeafNode* cur) {
  ...
  inline MemNode* removeLeafEntry(LeafNode* cur, uint slot) {
    ...
    for(uint i = slot + 1; i <= cur->num_keys; i++) {
  ...
  inline uint slotAtInner(uint64_t key, InnerNode* cur) {
  ...
  inline void removeInnerEntry(InnerNode* cur, uint slot, DeleteResult* res) {
    ...
    if (slot == 0) {
      ...
      for(uint i = slot; i < cur->num_keys - 1; i++) {
	cur->keys[i] = cur->keys[i + 1];
      }

    } else {
      //delete the previous key
      for(uint i = slot; i < cur->num_keys; i++) {
	cur->keys[i - 1] = cur->keys[i];
      }
    }
  ...
  inline DeleteResult* InnerDelete(uint64_t key, InnerNode* cur ,int depth)
  {
    ...
    if(res->upKey != (uint64_t)-1) {
      if (slot != 0) {
	cur->keys[slot - 1] = res->upKey;
	res->upKey = (uint64_t)-1;
      }
    }
  ...
  inline InnerNode* InnerInsert(uint64_t key, InnerNode *inner, int d, MemNode** val) {
	...
	if (k != (unsigned)-1) {
	  for (uint i=toInsert->num_keys; i>k; i--) {
	...
	//				checkConflict(inner, 0);
      ...
      __attribute__((unused)) bool s = true;
      InnerNode *new_inner =
  ...
  inline LeafNode* LeafInsert(uint64_t key, LeafNode *leaf, MemNode** val) {
    ...
    for (uint j=toInsert->num_keys; j>k; j--) {
